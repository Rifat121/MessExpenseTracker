...
router.post("/join", protect, asyncHandler(async (req, res) => {
  const { messname } = req.body;
...
router.post("/:messId/approve/:userId", protect, isAdmin, asyncHandler(async (req, res) => {
  const { messId, userId } = req.params;
  await messService.approveUser(messId, userId, req.user._id);
  res.json({ message: "User approved successfully" });
}));

router.get("/:messId/pending-members", protect, isAdmin, asyncHandler(async (req, res) => {
  const { messId } = req.params;
  const pendingUsers = await messService.getPendingMembers(messId, req.user._id);
  res.json(pendingUsers);
}));

router.delete("/:messId/reject/:userId", protect, isAdmin, asyncHandler(async (req, res) => {
  const { messId, userId } = req.params;
  await messService.rejectUser(messId, userId, req.user._id);
  res.json({ message: "User rejected and unlinked from mess" });
}));

router.delete("/:messId/remove/:userId", protect, isAdmin, asyncHandler(async (req, res) => {
  const { messId, userId } = req.params;
  await messService.removeUser(messId, userId, req.user._id);
  res.json({ message: "Member removed successfully" });
}));

router.get("/:messId", protect, asyncHandler(async (req, res) => {
  const { messId } = req.params;
